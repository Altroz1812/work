...
async function seedDatabase() {
  try {
    ...
    let tenant;
    if (existingTenant) {
      console.log('ℹ️ Demo tenant already exists, using existing tenant');
    ...
    if (existingUsers && existingUsers.length > 0) {
      console.log('ℹ️ Demo users already exist, skipping user creation');
    } else {
    ...
    const { data: existingWorkflow } = await supabase
      ...
      .select('*')
      .eq('workflow_id', 'micro_loan_v1')
      .eq('tenant_id', tenant.id)
      .single();

    if (existingWorkflow) {
      console.log('ℹ️ Demo workflow already exists, skipping workflow creation');
    } else {
      console.log('⚙️ Creating demo workflow...');
      ...
      if (!adminUser) {
        console.error('❌ Admin user not found for workflow creation');
        return;
      }
      ...
      const workflowConfig = {
        ...
        stages: [
          { id: 'draft', label: 'Draft', slaHours: 48, description: 'Initial loan application', assignToRoles: ['Maker'] },
          { id: 'doc_verify', label: 'Document Verification', slaHours: 24, description: 'Verify submitted documents', assignToRoles: ['Checker'] },
          { id: 'underwriting', label: 'Underwriting', slaHours: 48, description: 'Credit assessment and risk evaluation', assignToRoles: ['Underwriter'] },
          { id: 'approval', label: 'Approval', slaHours: 12, description: 'Final approval decision', assignToRoles: ['Admin', 'Underwriter'] },
          { id: 'disbursement', label: 'Disbursement', slaHours: 24, description: 'Loan disbursement process', assignToRoles: ['DisbursementOfficer'] },
    ...
    if (existingBorrowers && existingBorrowers.length > 0) {
      console.log('ℹ️ Demo borrowers already exist, skipping borrower creation');
    } else {
    ...
    console.log('✍️ Maker: maker@demo.com / maker123');
